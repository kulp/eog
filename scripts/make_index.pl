...
print
    ...
    p("Download a", a({ -href => "EOG_midi_pdf.zip" }, "zip file of all PDF and MIDI files"), "listed."),
            ...
            Tr(
                ...
                th({ -class => "sortbox", -colspan => 4 },
                    start_form({ -onSubmit => "return false;", -action => "" }),
                        input({ -id => "searchbox",
                                ...
                                -type => "text",
                                -size => 35 }),
                        button({ -onclick => "return do_reset();", -value => "Reset" }, "Reset"),
                    end_form,
                ),
             ...
             (map th({ -class => "wide", -colspan => $vcount{$_} }, $_), @dirs),
                Tr({ -class => "realhead" },
                    ...
                    (map th({ -class => "sorttable_nosort" }, $_), map @{ $variants{$_} }, @dirs),
                ),
            ),
        ),
        tbody(
            (map {
                ...
                Tr(
                    ...
                    td({ -class => "title", -sorttable_customkey => $safetitle }, $title),
                    td({ -class => "poet"                                      }, $poet),
